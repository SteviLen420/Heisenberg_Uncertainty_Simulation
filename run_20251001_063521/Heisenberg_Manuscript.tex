...
time achieves the theoretical minimum of $\hbar/2$. In addition, I simulate
...
where $\Delta x$ and $\Delta p$ are the standard deviations (uncertainties) of position
and momentum, respectively, and $\hbar$ is the reduced Planck constant. This
...
described by a complex-valued wave function $\psi(x,t)$ satisfying the
...
A Gaussian wave packet centered at position $x_0$ with mean momentum $p_0$ has
the analytical form:

\[\psi(x,0) = \left(\frac{1}{2\pi\sigma_x^2}\right)^{1/4} \exp\left[-\frac{(x-x_0)^2}{4\sigma_x^2} + \frac{ip_0(x-x_0)}{\hbar}\right]\]

where $\sigma_x$ is the initial position spread. For my simulations, I set $x_0 =
0$ and $p_0 = 0$ (particle at rest at the origin):
...
demonstrating the reciprocal relationship: as $\sigma_x$ increases, $\sigma_p$ decreases
proportionally.
...
For a Gaussian wave packet, $\Delta x = \sigma_x$ exactly.
...
For the Gaussian momentum distribution, $\Delta p = \hbar/(2\sigma_x)$.
...
The standard Heisenberg relation $\Delta x \cdot \Delta p \geq \hbar/2$ is actually a special case
...
For a Gaussian wave packet at t=0 with zero mean momentum ($p_0=0$),
Cov(x,p)=0 and the product saturates the minimal bound: $\Delta x \cdot \Delta p = \hbar/2$.

\textbf{Time evolution:} During free propagation, the momentum-position
correlation develops (Cov(x,p) $\neq$ 0 for $t>0$), causing the
uncertainty product to exceed $\hbar/2$ even though the wavefunction remains
...
minimum-uncertainty Gaussian wave packet with $\sigma_x(0)$ and zero initial
momentum ($p_0=0$)}, the position uncertainty evolves as:
...
This phase evolution, while not affecting $\langle p \rangle$ or $\Delta p$, is physically
...
Note that for $t>0$, the product \textbf{strictly exceeds} $\hbar/2$
because x-p correlations develop during free evolution. The
minimum-uncertainty property ($\Delta x \cdot \Delta p = \hbar/2$) holds only at t=0 for our
...
where L = 200 is the spatial domain size and $\Delta x = L/N$ is the grid
spacing. The wave function is sampled at these points: $\psi_j = \psi(x_j)$.
...
where $\Delta p = 2\pi\hbar/L$ is the momentum grid spacing.
...
\tightlist
\item
  Transform to momentum space: $\Psi(p) = \text{FFT}[\psi(x)]$
\item
  Apply kinetic evolution: $\Psi(p) \to \Psi(p) \exp[-iKdt/(2\hbar)]$
\item
  Transform back: $\psi(x) = \text{IFFT}[\Psi(p)]$
\item
...
\textbf{Grid Configuration:} - Number of points: N = $2^{14}$ = 16384 -
Spatial domain: $x \in [-100, 100]$ (atomic units) - Grid spacing: $\Delta x \approx
0.012$ - Momentum domain: $p \in [-\pi/\Delta x, \pi/\Delta x]$

\textbf{Initial Wave Packets:} - Center position: $x_0 = 0$ - Center
momentum: $p_0 = 0$ - Position spread range: $\sigma_x \in [0.5, 8.0]$ - Number
of $\sigma_x$ samples: 16 (geometrically spaced)

\textbf{Time Evolution:} - Particle mass: m = 1 (atomic units) - Time
step: dt = 0.002 - Total evolution time: t\_max = 1.0 - Number of steps:
500

\textbf{Unit System:} - Atomic units: $\hbar = 1$, m = 1 - All quantities
dimensionless
...
\item
  ...
  $\int |\psi|^2 dx \approx 1.0$ within $10^{-6}$
\item
...
functions, the standard deviation ($\sigma_x$) of which was systematically
varied.
...
demonstrate the inverse relationship between position uncertainty ($\Delta x$)
and momentum uncertainty ($\Delta p$). As I increased the standard deviation of
the initial wave packet in position space, i.e., the value of $\Delta x$, the
measured standard deviation in momentum representation, $\Delta p$, decreased
...
\textbf{Figure 1}: The value of the product $\Delta x \cdot \Delta p$ as a function of the
initial position's standard deviation, $\sigma_x$. The product is constant and
approximately 0.5, which, in atomic units (where $\hbar=1$), corresponds to
the theoretical minimum of $\hbar/2$.

The most significant result is that the product of these two quantities,
$\Delta x \cdot \Delta p$, remained constant across the investigated range at t=0. Based on
Figure 1 and the \texttt{heisenberg\_scan.csv} data, the value of the
product consistently hovers around 0.5. In an atomic unit system ($\hbar=1$),
this corresponds precisely to the minimum value allowed by the
Heisenberg relation, $\Delta x \cdot \Delta p \geq \hbar/2$, at the initial time.
...
representative state with an initial standard deviation of $\sigma_x = 3.0$.
Both distributions, as theoretically expected, have a Gaussian shape. A
wider distribution in position space (larger $\Delta x$) results in a narrower
distribution in momentum space (smaller $\Delta p$), visually confirming the
...
position space ($|\psi(x)|^2$) for an initial state with a
standard deviation of $\sigma_x = 3.0$.
...
momentum space ($|\Psi(p)|^2$) for an initial state with a
standard deviation of $\sigma_x = 3.0$.
...
uncertainty, $\Delta x(t)$, monotonically increases over time. This phenomenon
...
uncertainty $\Delta p$---remains constant in time. As a consequence, the
uncertainty product $\Delta x(t) \cdot \Delta p$ exceeds the minimum value $\hbar/2$ for
$t>0$ due to the development of position-momentum correlations.
...
\textbf{Figure 4}: The change in position uncertainty $\Delta x(t)$ over time
for a freely propagating Gaussian wave packet. The wave packet spreads
out in time, resulting in an increase in $\Delta x$.
...
uncertainty principle ($\Delta x \cdot \Delta p \geq \hbar/2$) at the initial time. The data shows
...
achieves the theoretical limit of $\hbar/2$, which is the defining property of
...
product $\Delta x \cdot \Delta p$ assumes the minimum possible value, $\hbar/2$, at t=0 means that
...
($\Delta x(t)$) increases during free evolution, the momentum uncertainty ($\Delta p$)
remains constant. Consequently, the uncertainty product, $\Delta x(t) \cdot \Delta p$, also
increases over time and strictly exceeds $\hbar/2$ for $t>0$. This is
...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Quantitative verification}: The uncertainty product $\Delta x \cdot \Delta p$
  consistently equals $\hbar/2$ (0.5 in atomic units) at t=0 across all
...
\item
  ...
  exhibit the predicted inverse proportionality $\Delta p = \hbar/(2\Delta x)$,
...
\item
  ...
  growth of position uncertainty $\Delta x(t)$ while momentum uncertainty $\Delta p$
  remains constant, consistent with the analytical prediction for free
  Gaussian packets. The uncertainty product exceeds $\hbar/2$ for
  $t>0$ due to correlation development.
\item
...
\NormalTok{    product }\OperatorTok{=}\NormalTok{ Delta\_x }\OperatorTok{*}\NormalTok{ Delta\_p}
    \BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f"sigma\_x=}\SpecialCharTok{\{}\NormalTok{sigma\_x}\SpecialCharTok{:.2f\}}\SpecialStringTok{: Delta\_x*Delta\_p=}\SpecialCharTok{\{}\NormalTok{product}\SpecialCharTok{:.4f\}}\SpecialStringTok{"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\item
  Full $\sigma_x$ scan (16 points): \textasciitilde160 ms
\item
